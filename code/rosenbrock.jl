...
# ╔═╡ cf4039cc-5c40-11ec-077d-172f79f6238c
begin
	using Plots
...
# ╔═╡ 59d29255-52ee-4c45-88f7-1635db9e4e5c
md"# Rosenbrock's function and ODE solvers"

# ╔═╡ 94fea46a-ef39-40a3-acba-0278a6cdc612
md"""
Rosenbrock's function is the classical example of an ill-behaved optimization problem. In this example, we use the fact that classical optimization methods are basically discretizations of an ODE, and compare the solution of that ODE using solvers from DifferentialEquations with the trajectories we obtain using standard optimizers.
"""

# ╔═╡ 39c574cc-b210-479b-8013-f22412462d20
rosenbrock(x) = (1-x[1])^2 + 100*(x[2]-x[1]^2)^2

# ╔═╡ 00eaedef-c537-4a43-ab77-6d0e524c43a3
md"""
Using ForwardDiff, we can compute the gradient of our function using Automatic Differentiation.
"""

# ╔═╡ e6b64b92-e01e-44ef-98ca-9c651f215764
∇f(x) = - ForwardDiff.gradient(rosenbrock, x)

# ╔═╡ ee5e5838-f2d2-407c-b4b0-4b41caed9b74
md"""
To solve ODEs, it's better to use the in-place form, as it doesn't allocate memory.
"""

# ╔═╡ e45cc301-86c1-4689-86bd-559126e881e9
function ∇f!(dx, x, p, t)
    ForwardDiff.gradient!(dx, rosenbrock, x)
    dx .*= -1
end

# ╔═╡ a0afed04-b7b5-4728-8c3e-4afe699d88b6
x₀ = [-1.0,-1.0]

# ╔═╡ 131db100-7461-4d6f-bbb5-2f82a82514c2
∇f(x₀)

# ╔═╡ 177f7861-2b63-4290-a499-4e458bab06c5
md"""
We begin by solving the ODE problem we defined above using various stiff and non-stiff solvers.
"""

# ╔═╡ 8b539f4c-8904-44e2-96f3-a22f4885aabc
begin
	prob = ODEProblem(∇f!, x₀, (0.0, 20.0))

	sol_0 = solve(prob, Euler(), dt=0.002)
	sol_1 = solve(prob, Tsit5()) # non-stiff solver
	sol_2 = solve(prob, ImplicitEuler()) # stiff solver
	sol_3 = solve(prob, Rosenbrock23()) # stiff solver
	nothing
end

# ╔═╡ 4b3567c4-a358-47b9-8e33-d6dfb9191ab3
md"Next, we optimize the rosenbrock function using gradient descent and Newton's method"

# ╔═╡ cc3c00cc-7f43-4f3f-9d55-83321e95ebb4
begin
	res = Optim.optimize(
	    rosenbrock, x₀, 
	    GradientDescent(), 
	...
	trace = Optim.x_trace(res)
	trace_x = [x[1] for x in trace]
	trace_y = [x[2] for x in trace]
end

# ╔═╡ 4cc7b07b-a6ec-4012-911f-0565b8fbdd3b
begin
	res_n = Optim.optimize(
    	rosenbrock, x₀, 
	    Newton(), 
	    Optim.Options(
	        iterations=10000,
	...
	trace_n = Optim.x_trace(res_n)
	trace_n_x = [x[1] for x in trace_n]
	trace_n_y = [x[2] for x in trace_n]
end

# ╔═╡ 76183fd8-6cea-424b-9031-124cb009b013
md"""
Looking at the trajectories, they seem pretty wild, even for the ODE solvers. Moreover, the non-stiff solvers are taking a large amount of steps for such a simple problem. As it turns out, the Rosenbrock ODE is stiff!
"""

# ╔═╡ c2d734f5-85be-49ac-be25-bee4853996ce
begin
	x = y = range(-2.0,2.0, length=200)
    contour(x, y, (x, y) -> rosenbrock([x, y]), 
        legend=:topleft,
        xlabel="x",
        ylabel="y"
        )
    scatter!((1.0,1.0), label="Global optimum")
    plot!(trace_x, trace_y, label="GradientDescent ($(Optim.iterations(res)) steps)")
    plot!(trace_n_x, trace_n_y, label="Newton ($(Optim.iterations(res_n)) steps)")
    plot!(sol_0, vars=(1,2), label="Euler ($(length(sol_0.t)) steps)")
    plot!(sol_1, vars=(1,2), label="Tsit5 ($(length(sol_1.t)) steps)")
    plot!(sol_2, vars=(1,2), label="ImplicitEuler ($(length(sol_2.t)) steps)")
    plot!(sol_3, vars=(1,2), label="Rosenbrock23 ($(length(sol_3.t)) steps)")
end

# ╔═╡ c3ddc4a3-c20a-4832-81c6-40fedc352b6a
md"""
If we compare the distance to the true solution as a function of the number of steps taken, the stiff solvers show a similar curve to Newton's method. This isn't too surprising, as they involve solving a Jacobian problem which is more or less equivalent to the Hessian problem in Newton's method.
"""

# ╔═╡ fdecce89-2402-4725-8e72-5ff401519403
begin
    dist(x) = norm(x - [1,1])
    plot(map(dist, trace),
        xscale=:log10,
        ...
        legend=:bottomright,
        xlabel="#steps",
        ylabel="Distance from global optimum"
    )
    plot!(map(dist, trace_n),
        label="Newton"
    )
    plot!(map(dist, sol_0.u), label="Euler")
    plot!(map(dist, sol_1.u), label="Tsit5")
    plot!(map(dist, sol_2.u), label="ImplicitEuler")
    plot!(map(dist, sol_3.u), label="Rosenbrock23")
end
